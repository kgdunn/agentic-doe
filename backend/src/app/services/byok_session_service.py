...
def orphan_dek(user: User) -> bool:
    ...
    Returns True if the user had a non-terminal enrollment (``active``
    or ``rejected``) that was just orphaned, False otherwise. Idempotent
    — calling on an already-orphaned or absent row is a no-op.
    """
