...
async def chat(
    ...
    Returns an SSE stream. Event names the client can observe:
    ``conversation_id``, ``phase``, ``plan``, ``plan_update``,
    ``token``, ``tool_start``, ``tool_result``, ``experiment_created``,
    ``simulator_created``, ``done``, and ``error``.
    """
