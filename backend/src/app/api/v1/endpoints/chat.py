...
async def chat(
    ...
    Returns an SSE stream with events: ``conversation_id``, ``phase``,
    ``plan``, ``plan_update``, ``token``, ``tool_start``, ``tool_result``,
    ``experiment_created``, ``simulator_created``, ``interrupted``,
    ``done``, and ``error``.
    """
...
async def get_conversation_messages(
    ...
    ``tool_result`` rows are grouped into a synthetic message whose role
    is rewritten from ``user`` to ``assistant`` for the frontend.
    """
    conversation = await db.get(Conversation, conversation_id)
    if not conversation:
        raise HTTPException(status_code=404, detail="Conversation not found")
